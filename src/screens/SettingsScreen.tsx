...
interface Props {
  ...
  onSetLanguage: (lang: 'en' | 'es' | 'fr' | 'ja' | 'id' | 'ar') => void;
  onReset: () => void;
  onBack: () => void;
}
      ...
      <div className="flex-1 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className="p-4 bg-white border border-[#E5E7EB] rounded-2xl flex flex-col gap-2.5 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-2 mt-1">
            {(['en', 'es', 'fr', 'ja', 'id', 'ar'] as const).map(lang => (
              <button
                key={lang}
              ...
              >
                {lang === 'en' ? 'English' : lang === 'es' ? 'Español' : lang === 'fr' ? 'Français' : lang === 'ja' ? '日本語' : lang === 'id' ? 'Indonesia' : 'العربية'}
              </button>
            ))}
          </div>
        </div>
